...
PIDimp::PIDimp(Servo * myServo, AS5050 * myEncoder) : encVals({0, 0, 0, 0, 0}){
  servo = myServo;
  encoder = myEncoder;
  sum = 0.0;
  indexer = 0;
...
float PIDimp::getPosition( ){
	sum -= encVals[indexer];
	encVals[indexer] = (float)encoder->totalAngle();// the angle is divided by 100 to get rid of noise/ random jitters
	sum += encVals[indexer];
	indexer = (indexer +1)%5;
    float a= sum/5;

   return (a); //(float)encoder->totalAngle();;
}
